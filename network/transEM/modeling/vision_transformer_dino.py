...
class Block_adapter(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        if return_attention:
            return attn
        x = x + self.drop_path(y)
        x = x + self.drop_path(self.mlp(self.norm2(x))) + self.drop_path(self.adapter(self.norm2(x)))
        return x
...
class Adapter_Layer(nn.Module):
    ...
    def forward(self, x):
        #x -> （B, H, W, C）-> （B, C, H, W）
        #NEW x (B,HW,C)-> （B, C, H, W）
        # x = x.permute(0,3,1,2)
        x = x.permute(0,2,1).view(x.size(0), x.size(2), int(math.sqrt(x.size(1))), int(math.sqrt(x.size(1))))
        B, C, _, _ = x.size()
...
def vit_small(patch_size=16, checkpoint = None, **kwargs):
    model = VisionTransformer(
    ...
    if(patch_size == 16):
        ...
        if(checkpoint == None):
            ...
            state_dict = torch.load('model/dino_deitsmall8_pretrain.pth')
